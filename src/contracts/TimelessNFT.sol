...
contract TimelessNFT is ERC721Enumerable, Ownable {
    // generating for token Id (library)
    using Strings for uint256;
    mapping(string => uint8) existingURIs; // after minted data goes here
    mapping(uint256 => address) public holderOf; // who is holding the particular NFT

    address public artist;
    uint256 public royalityFee;
    uint256 public supply = 0; // total available supply
    uint256 public totalTx = 0; // transaction total
    uint256 public cost = 0.01 ether; // cost for the mint

    event Sale(
        // when minted this event will fire
        uint256 id,
    ...
    struct TransactionStruct {
        // holding the transaction information
        uint256 id;
        address owner;
        uint256 cost;
        string title;
    ...
    constructor(
        // definig Constructor parameters
        string memory _name,
    ...
    function payToMint(
        ...
        payTo(owner(), (msg.value - royality)); // pay to mint to the owner
